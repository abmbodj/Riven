...
serve(async (request) => {
  ...
  try {
    ...
    if (!stripeCustomerId) {
      return jsonResponse({ error: 'No Stripe customer record found. Please make a purchase first.' }, { status: 404 });
    }
    ...
    return jsonResponse({ error: error?.message || 'Failed to create portal session' }, { status });
  }
});
