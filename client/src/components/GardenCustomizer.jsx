...
import { Palette, Sparkles, Trees, Lock, Check } from 'lucide-react';
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="w-full h-[92dvh] sm:h-auto sm:max-w-3xl sm:max-h-[85vh] overflow-hidden rounded-t-2xl sm:rounded-2xl flex flex-col bg-claude-surface text-claude-text"
                ...
                >
                    ...
                    {/* Header */}
                    <div className="flex items-center justify-between px-5 py-3 sm:p-6 border-b border-claude-border shrink-0">
                        <div>
                            <h2 className="text-lg sm:text-2xl font-display font-bold">Customize Garden</h2>
                            <p className="text-xs sm:text-sm text-claude-secondary mt-0.5">
                    ...
                    <div className="flex flex-col sm:flex-row flex-1 overflow-hidden">
                        {/* Preview Panel */}
                        <div className="w-full sm:w-1/3 p-4 sm:p-6 flex flex-col items-center justify-center border-b sm:border-b-0 sm:border-r border-claude-border bg-claude-bg/50">
                            <Garden
                        ...
                        <div className="flex-1 flex flex-col overflow-hidden">
                            ...
                            <div className="flex border-b border-claude-border shrink-0">
                                {tabs.map(tab => (
                                    <button
                                        key={tab.id}
                                        className={`flex-1 p-3 sm:p-4 flex flex-col sm:flex-row items-center justify-center gap-1.5 sm:gap-2 transition-all relative ${activeTab === tab.id ? 'text-claude-accent' : 'text-claude-secondary active:text-claude-text'
                                            }`}
                                                        ...
                                                        {slotDecorations.map(dec => {
                                                            ...
                                                            return (
                                                                <button
                                                                    ...
                                                                    className={`p-3 rounded-xl border-2 transition-all ${isEquipped
                                                                            ? 'border-claude-accent bg-claude-accent/10'
                                                                            : isUnlocked
                                                                                ? 'border-claude-border active:border-claude-secondary'
                                        ...
                                        {specialPlants.map(plant => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    disabled={!isUnlocked && !isEquipped}
                                                    className={`p-4 rounded-xl border-2 transition-all ${isEquipped
                                                            ? 'border-claude-accent bg-claude-accent/10'
                                                            : isUnlocked
                                                                ? 'border-claude-border active:border-claude-secondary'
                                                                : 'border-claude-border/50 opacity-50'
